...
import com.androidas.lib.util.ArrayUtils;
import com.androidas.lib.util.MathUtils;
import com.androidas.lib.util.SharedPreferencesUtils;
import com.cheng.game2358.R;
import com.cheng.game2358.common.Common;
import com.cheng.game2358.ui.fragment.AboutFragment;
...
//    private static final int DEFAULT_COLUMN = 6;
     ...
     * GridLayout列数
     */
    private int mColumn = AboutFragment.CHENG;
    /**
     * item
     */
    ...
    private void initView() {
        /** 初始化GridLayout */
        setColumnCount(AboutFragment.CHENG);
        setFocusable(true);
...
//                canvas.drawCircle(mGap + (i / mColumn) * (mItemSize + mGap), mGap + (i % mColumn) * (mItemSize + mGap),mRadius,mPaint);
            } else {
        ...
        for (int i = 0; i < mColumn; i++) {
            ...
            for (int j = 0; j < row.length; j++) {
                if (row[j] == 0) {
                    ...
                    canMove = true;
                    break;
                } else if (j - 1 >= 0 && (row[j - 1] == row[j] && ((row[j - 1] / row[j] < 0.7 && row[j - 1] / row[j] > 0.6) || (row[j] / row[j - 1] < 0.7 && row[j] / row[j - 1] > 0.6)))) {
                    canMove = true;
                    break;
                }
            }
...
//            int n=i;int ff= (int )((Math.sqrt(5)/5)*( Math.pow((1+Math.sqrt(5)/2),n)-Math.pow((1-Math.sqrt(5))/2,n)));
            float aa = (float) col[i - 1] / col[i];
            float bb = (float) col[i] / col[i - 1];

            float cc = (float) col[i - move[i]] / col[i];
            float dd = (float) col[i] / col[i - move[i]];
            if (col[i - 1] != 0 && col[i] == 0) {
                ...
                move[i] = move[i - 1] + 1;
                if (col[i] != 0 && col[i - move[i]] != 0 && (col[i] == col[i - move[i]] ||col[i] ==col[i]+  col[i - move[i]]||col[i-move[i]] ==col[i]+  col[i - move[i]]
                        ||col[i] ==col[i]+ col[i - move[i]]+ col[i - move[i]]||col[i-move[i]] == col[i - move[i]]+col[i]+col[i]
                        ||col[i] ==col[i]+ col[i - move[i]]+ col[i - move[i]]+ col[i - move[i]]||col[i-move[i]] == col[i - move[i]]+col[i]+col[i]+col[i]
                        || !((cc > 0.6 && cc < 0.7) || (dd > 0.6 && dd < 0.7)))) {
//                if(col[i]!=0 && col[i-move[i]]!=0 && (col[i]==col[i-move[i]]||((col[i-move[i]]/col[i]<0.6||col[i-move[i]]/col[i]>0.7)||(col[i]/col[i-move[i]]<0.6||col[i]/col[i-move[i]]>0.7)))){
                    move[i] = move[i] + move[i - move[i]];
                    move[i] = move[i] - 1;
                }
                else if(col[i] == col[i - move[i]] ||col[i] ==col[i]+  col[i - move[i]]||col[i-move[i]] ==col[i]+  col[i - move[i]]
                        ||col[i] ==col[i]+ col[i - move[i]]+ col[i - move[i]]||col[i-move[i]] == col[i - move[i]]+col[i]+col[i]
                        ||col[i] ==col[i]+ col[i - move[i]]+ col[i - move[i]]+ col[i - move[i]]||col[i-move[i]] == col[i - move[i]]+col[i]+col[i]+col[i]){
                    move[i] = move[i] + move[i - move[i]];
                    move[i] = move[i] - 1;
                ...
                else if (flag != i - move[i] && (col[i] != col[i - move[i]]||col[i] !=col[i]+  col[i - move[i]]||col[i-move[i]] !=col[i]+  col[i - move[i]]
                        ||col[i] !=col[i]+ col[i - move[i]]+ col[i - move[i]]||col[i-move[i]] != col[i - move[i]]+col[i]+col[i]
                        ||col[i] !=col[i]+ col[i - move[i]]+ col[i - move[i]]+ col[i - move[i]]||col[i-move[i]] != col[i - move[i]]+col[i]+col[i]+col[i])) {
                    move[i] = move[i] + move[i - move[i]];
//                    if (col[i] == col[i - move[i]]) {
                    if (col[i] != col[i - move[i]]||col[i] !=col[i]+  col[i - move[i]]||col[i-move[i]] !=col[i]+  col[i - move[i]] || ((cc > 0.6 && cc < 0.7) || (dd > 0.6 && dd < 0.7))) {
                        flag = i;
                    }
                }
            }
//            col[i-1]==col[i]
//            float aa=col[i-1]/col[i];float bb=col[i]/col[i-1];

            else if (((col[i - 1] != col[i]||col[i] !=col[i]+  col[i - move[i]]||col[i-move[i]] !=col[i]+  col[i - move[i]]
                    ||col[i] !=col[i]+ col[i - move[i]]+ col[i - move[i]]||col[i-move[i]] != col[i - move[i]]+col[i]+col[i]
                    ||col[i] !=col[i]+ col[i - move[i]]+ col[i - move[i]]+ col[i - move[i]]||col[i-move[i]] != col[i - move[i]]+col[i]+col[i]+col[i]
                    ) && (aa > 0.6 && aa < 0.7) || (bb > 0.6 && bb < 0.7)) && flag != i - 1) {
//            else if  ((col[i-1]/col[i]>0.6&&col[i-1]/col[i]<0.7)||(col[i]/col[i-1]>0.6&&col[i]/col[i-1]<0.7) && flag!=i-1){
                move[i] = move[i - 1] + 1;
                flag = i;
            } else {
                move[i] = move[i - 1];
        ...
        for (int i = 1; i <= last; i++) {
            ...
            Random r = new Random();
            int n = r.nextInt(100);
            int ff = (int)((Math.sqrt(5) / 5) * (Math.pow((1 + Math.sqrt(5) / 2), n) - Math.pow((1 - Math.sqrt(5)) / 2, n)));
...
//                if ((row[j-1]/row[j]<0.7&&row[j-1]/row[j]>0.6)||(row[j]/row[j-1]<0.7&&row[j]/row[j-1]>0.6))
//                if ((row[j-1]/row[j]<0.7&&row[j-1]/row[j]>0.6)||(row[j]/row[j-1]<0.7&&row[j]/row[j-1]>0.6))
//                if (col[i]/col[i-move[i]]>0.6&&col[i]/col[i-move[i]]<0.7) {
//                    col[i - move[i]] = col[i - move[i]] + col[i];
//                    col[i] = 0;
//                }
//                if (col[i-move[i]]/col[i]>0.6&&col[i-move[i]]/col[i]<0.7) {
//                    col[i - move[i]] += col[i];
//                    col[i] = 0;
//                }
//                col[i-move[i]] += col[i];

//                ArrayUtils.contains(arr[a],col[i - move[i]]);
//                int arr[] = new int[66];
//                arr[0] = arr[1] = 1;
...
//                    arr[a] = arr[a - 1] + arr[a - 2];
    ...
    public void addRandomNum() {
        if(!isFull()) {
            ...
            mItems[index].setNumber(Math.random() > 0.6d ? 2 : 3);
            nullIndexs.remove(Integer.valueOf(index));
        } else {/** 如果已经填满了, 游戏结束 */
            isGameStart = false;
            onGameListener.onGameOver();
